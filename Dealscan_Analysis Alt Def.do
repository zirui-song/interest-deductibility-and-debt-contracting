...
preserve 
	keep if treated_loss > 0
	keep gvkey conm year 
	export delimited "../3. Data/Processed/lossfirm_list.csv", replace
restore
...
reghdfe not_rated treated1 treated1_post treated_loss treated_loss_post `controls' `deal_controls', absorb(year ff_48) vce(cluster gvkey)
estimates store m2
preserve 
	drop if sp_rating_num == 0
	reghdfe sp_rating_num treated1 treated1_post treated_loss treated_loss_post `controls' `deal_controls', absorb(year ff_48) vce(cluster gvkey)
	estimates store m1
restore
...
reghdfe margin_bps `treat_vars' `controls' `deal_controls' sp_rating_num, absorb(year ff_48) vce(cluster gvkey)
reghdfe margin_bps `treat_vars' `controls' `controls_post' `deal_controls' sp_rating_num, absorb(year ff_48) vce(cluster gvkey)

reghdfe margin_bps `treat_vars' `deal_controls' sp_rating_num, absorb(year gvkey) vce(cluster gvkey)
reghdfe margin_bps `treat_vars' `controls' `deal_controls' sp_rating_num, absorb(year gvkey) vce(cluster gvkey)
reghdfe margin_bps `treat_vars' `controls' `controls_post' `deal_controls' sp_rating_num, absorb(year gvkey) vce(cluster gvkey)


reghdfe margin_bps treated2 treated2_post `controls' `controls_post' `deal_controls' sp_rating_num, absorb(year ff_48) vce(cluster gvkey)
...
reghdfe perf_pricing_dummy `treat_vars' `controls' `deal_controls', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m1
reghdfe num_fin_cov `treat_vars' `controls' `deal_controls', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m2
*** DID regressions (covenant tightness)
reghdfe pviol `treat_vars' `controls' `deal_controls', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m3
...
reghdfe deal_amount_converted `treat_vars' `controls' `deal_controls_2', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m4

reghdfe log_deal_amount_converted `treat_vars' `controls' `deal_controls_2', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m5

reghdfe maturity `treat_vars' `controls' `deal_controls_2', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m6
