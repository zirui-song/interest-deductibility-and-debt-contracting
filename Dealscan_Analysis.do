...
estimates store m3
	reghdfe not_rated `treat_quartiles' `controls' `deal_controls', absorb(year ff_48) vce(cluster gvkey)
estimates store m4
preserve 
	drop if sp_rating_num == 0
	...
	estimates store m1
	reghdfe sp_rating_num `treat_quartiles' `controls' `deal_controls', absorb(year ff_48) vce(cluster gvkey)
	estimates store m2
restore
* save the results (esttab) using overleaf_dir
esttab m1 m2 m3 m4 using "$overleaf_dir/sp_rating_continuous.tex", replace ///
nodepvars nomti nonum collabels(none) label b(3) se(3) parentheses ///
...
reghdfe perf_pricing_dummy excess_interest_scaled excess_interest_scaled_post `controls' `deal_controls', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m1
reghdfe num_fin_cov excess_interest_scaled excess_interest_scaled_post `controls' `deal_controls', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m2
*** DID regressions (covenant tightness)
reghdfe pviol excess_interest_scaled excess_interest_scaled_post `controls' `deal_controls', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m3
...
reghdfe deal_amount_converted excess_interest_scaled excess_interest_scaled_post `controls' `deal_controls_2', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m4

reghdfe log_deal_amount_converted excess_interest_scaled excess_interest_scaled_post `controls' `deal_controls_2', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m5

reghdfe maturity excess_interest_scaled excess_interest_scaled_post `controls' `deal_controls_2', absorb(year ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m6

* save the results (esttab) using overleaf_dir
esttab m1 m2 m3 m4 m5 m6 using "$overleaf_dir/other_terms_ie_excess.tex", replace ///
nodepvars nomti nonum collabels(none) label b(3) se(3) parentheses ///
...
reg margin_bps excess_interest_scaled excess_interest_scaled_post `controls' `deal_controls' i.year i.ff_48 ib2.sp_rating_num if high_competition == 1
estimates store m1

reg margin_bps excess_interest_scaled excess_interest_scaled_post `controls' `deal_controls' i.year i.ff_48 ib2.sp_rating_num if high_competition == 0
estimates store m2
