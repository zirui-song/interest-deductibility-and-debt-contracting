...
estimates store m2
reghdfe margin_bps excess_interest_scaled excess_interest_scaled_post `controls' `controls_post' `deal_controls', absorb(yea##ff_48 sp_rating_num) vce(cluster gvkey)
estimates store m3

* save the results (esttab) using overleaf_dir
esttab m1 m2 m3 using "$overleaf_dir/margin_did_both_rule_falsification_cts.tex", replace ///
nodepvars nomti nonum collabels(none) label b(3) se(3) parentheses ///
star(* 0.10 ** 0.05 *** 0.01) ar2 plain lines fragment noconstant drop(_cons `controls_post')
est clear
